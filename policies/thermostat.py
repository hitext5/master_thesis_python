def eval_policy_ac_on(requesting_device, collection):
    ...
    weather_station = collection.find_one({"device_type": "weather_station"})
    # Retrieve smartphone data from database
    smartphone = collection.find_one({"device_type": "smartphone"})
...
actions_dict = {
    'eval_policy_ac_on': [{'device': 'window', 'to_do': 'open_window'},
